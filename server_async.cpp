...
public:
    AsyncQpsServerTest(const ServerConfig &config,
            ...
            std::function<
                    void(ServiceType *, ServerContextType *, RequestType *,
                            ServerAsyncResponseWriter<ResponseType> *,
                            CompletionQueue *, ServerCompletionQueue *, void *)> request_unary_function,
            std::function<
                    void(ServiceType *, ServerContextType *,
                            ServerAsyncReaderWriter<ResponseType, RequestType> *,
                            CompletionQueue *, ServerCompletionQueue *, void *)> request_streaming_function,
            std::function<
                    grpc::Status(const PayloadConfig &, const RequestType *,
        ...
        auto process_rpc_bound = std::bind(process_rpc, config.payload_config(), _1, _2);

        for (int i = 0; i < 15000; i++) {
            for (int j = 0; j < num_threads; j++) {
                if (request_unary_function) {
                    auto request_unary = std::bind(request_unary_function,
                            &async_service_, _1, _2, _3, srv_cqs_[j].get(),
                            srv_cqs_[j].get(), _4);
                    contexts_.emplace_back(
                            new ServerRpcContextUnaryImpl(request_unary,
                                    process_rpc_bound));
                }
                if (request_streaming_function) {
                    auto request_streaming = std::bind(
                            request_streaming_function, &async_service_, _1, _2,
                            srv_cqs_[j].get(), srv_cqs_[j].get(), _3);
                    contexts_.emplace_back(
                            new ServerRpcContextStreamingImpl(request_streaming,
                                    process_rpc_bound));
                }
            }
        }
...
static Status ProcessSimpleRPC(const PayloadConfig &,
    ...
    if (request->response_size() > 0) {
        if (!Server::SetPayload(request->response_type(),
                request->response_size(), response->mutable_payload())) {
...
std::unique_ptr<Server> CreateAsyncServer(const ServerConfig &config) {
    return std::unique_ptr<Server>(
            new AsyncQpsServerTest<SimpleRequest, SimpleResponse,
                    BenchmarkService::AsyncService, grpc::ServerContext>(config,
...
std::unique_ptr<Server> CreateAsyncGenericServer(const ServerConfig &config) {
    return std::unique_ptr<Server>(
            new AsyncQpsServerTest<ByteBuffer, ByteBuffer,
                    grpc::AsyncGenericService, grpc::GenericServerContext>(
