...
static std::unordered_map<string, std::deque<int>> get_hosts_and_cores(const deque<string>& workers) {
    std::unordered_map<string, std::deque<int>> hosts;
    for (auto it = workers.begin(); it != workers.end(); it++) {
        const string host = get_host(*it);
        if (hosts.find(host) == hosts.end()) {
            auto stub = WorkerService::NewStub(CreateChannel(*it, InsecureChannelCredentials()));
...
static double WallTime(ClientStats s) {
    return s.time_elapsed();
}
static double SystemTime(ClientStats s) {
    return s.time_system();
}
static double UserTime(ClientStats s) {
    return s.time_user();
}
static double ServerWallTime(ServerStats s) {
    return s.time_elapsed();
}
static double ServerSystemTime(ServerStats s) {
    return s.time_system();
}
static double ServerUserTime(ServerStats s) {
    return s.time_user();
}
        ...
        int spawn_local_worker_count)
{
    ...
    for (size_t i = 0; i < num_servers; i++) {
        ...
        servers[i].stub = WorkerService::NewStub(CreateChannel(workers[i], InsecureChannelCredentials()));
    ...
    for (size_t i = 0; i < num_clients; i++) {
        ...
        clients[i].stub = WorkerService::NewStub(CreateChannel(worker, InsecureChannelCredentials()));
        ...
        size_t num_channels = (client_config.client_channels() - channels_allocated) / (num_clients - i);
    ...
    gpr_sleep_until(
            gpr_time_add(start,
                    gpr_time_from_seconds(warmup_seconds, GPR_TIMESPAN)));
