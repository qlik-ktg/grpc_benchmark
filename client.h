...
public:
    ClientRequestCreator(ByteBuffer* req, const PayloadConfig& payload_config) {
        if (payload_config.has_bytebuf_params()) {
            ...
            gpr_slice s = gpr_slice_from_copied_buffer(buf.get(),
...
public:
    Client()
            : timer_(new UsageTimer), interarrival_timer_() {
    }
    virtual ~Client() {
    }
...
protected:
        ...
        if (!random_dist) {
            ...
            for (size_t i = 0; i < num_threads; i++) {
                next_time_.push_back(
                        gpr_time_add(now,
                                gpr_time_from_nanos(interarrival_timer_.next(i),
                                        GPR_TIMESPAN)));
            }
        }
    }
    ...
    gpr_timespec NextIssueTime(int thread_idx) {
        ...
        next_time_[thread_idx] = gpr_time_add(next_time_[thread_idx],
    ...
    private:
        ...
        void ThreadFunc() {
            for (;;) {
                ...
                if (!thread_still_ok
                        || static_cast<bool>(gpr_atm_acq_load(
                                &client_->thread_pool_done_))) {
...
public:
    ClientImpl(const ClientConfig& config,
            ...
            : cores_(
                    LimitCores(config.core_list().data(),
                            config.core_list_size())), channels_(
                    config.client_channels()), create_stub_(create_stub) {
        for (int i = 0; i < config.client_channels(); i++) {
            channels_[i].init(
                    config.server_targets(i % config.server_targets_size()),
...
protected:
    ...
    public:
        ...
        void init(const grpc::string& target, const ClientConfig& config,
                std::function<
                        std::unique_ptr<StubType>(std::shared_ptr<Channel>)> create_stub,
                int shard) {
            ...
            channel_ = CreateTestChannel(target,
...
std::unique_ptr<Client> CreateSynchronousStreamingClient( const ClientConfig& args);
